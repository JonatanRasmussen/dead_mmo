...
from ._controls_system import ControlsSystem, ObjControlsData
...
class OldWorldState:
    ...
    def __init__(self) -> None:
        ...
        self._new_event_id_gen: IdGen = IdGen.create_preassigned_range(1, 100_000)  # Doubled during refactor to ensure old and new system creates same event IDs
    ...
    def process_frame(self, player_inputs: list[str], frame_end: int) -> None:
        ...
        for game_obj in self.view_game_objs:
            ...
            OldWorldState.debug_compare_ecs_to_gameobj(
                self._combat_system.game_obj_combat_dct[obj_id],
                self._movement_system.game_obj_positions_dct[obj_id],
                self._targeting_system.game_obj_targeting_dct[obj_id],
                self._controls_system.game_obj_controls_dct[obj_id],
                game_obj,
                frame_end
            )
    ...
    def _finalize_and_process_event(self, u_event: UpcomingEvent) -> UpcomingEvent:
        ...
        #
        new_system_targ_id = self._targeting_system.decide_targeting(u_event.target_id, u_event.is_aoe_targeting, source_obj.obj_id, u_event.spell_id)
        assert target_obj.obj_id == new_system_targ_id, f"Mismatched targeting: {target_obj.obj_id} != {new_system_targ_id} for event {u_event}, also {self._default_ids} compared to {self._targeting_system.default_ids}"
        #
        expired_aura = u_event.is_aura_tick and not self._auras.aura_exists(u_event.aura_id)
        outcome = OldWorldState._decide_outcome(u_event.timestamp, source_obj, spell, target_obj, expired_aura, u_event.is_aoe_targeting)
        new_systems_outcome = self._new_decide_outcome(u_event.timestamp, u_event.source_id, u_event.spell_id, new_system_targ_id, expired_aura, u_event.is_aoe_targeting)
        if outcome != Outcome.OUT_OF_RANGE and new_systems_outcome != Outcome.OUT_OF_RANGE:  # Ignore due to slight difference in obj_positions between the two systems
            assert outcome == new_systems_outcome, f"Mismatched outcome: {outcome} != {new_systems_outcome} for event {u_event}"
        f_event = u_event.finalize_event(source_obj.obj_id, target_obj.obj_id, outcome)
        self._new_process_event(f_event, source_obj, spell, target_obj)
        return f_event


    def _new_process_event(self, f_event: UpcomingEvent, source_obj: GameObj, spell: Spell, target_obj: GameObj) -> None:
        ...
        if f_event.outcome_is_valid:
            ...
            if spell.has_aura_apply:
                new_aura_id = self._auras.add_aura(timestamp, source_id, spell.spell_id, target_id)

            if spell.has_cascading_events:
                # --- PARALLEL TEST BLOCK ---
                new_obj_id = new_obj.obj_id if new_obj is not None else None

                # Consume both generators into lists
                old_events = list(self._fetch_cascading_events(f_event, new_obj, source_obj, spell, target_obj, new_aura_id))
                new_events = list(self._new_fetch_cascading_events(f_event, new_obj_id, source_id, spell.spell_id, target_id, new_aura_id))

                # 1. Compare lengths
                assert len(old_events) == len(new_events), f"Cascading event count mismatch: {len(old_events)} (old) != {len(new_events)} (new)"

                # 2. Compare individual event fields (ignoring event_id)
                for old_ev, new_ev in zip(old_events, new_events):
                    assert old_ev.timestamp == new_ev.timestamp, f"Timestamp mismatch: {old_ev.timestamp} != {new_ev.timestamp}"
                    assert old_ev.source_id == new_ev.source_id, f"Source ID mismatch: {old_ev.source_id} != {new_ev.source_id}"
                    assert old_ev.spell_id == new_ev.spell_id, f"Spell ID mismatch: {old_ev.spell_id} != {new_ev.spell_id}"
                    assert old_ev.target_id == new_ev.target_id, f"Target ID mismatch: {old_ev.target_id} != {new_ev.target_id}"
                    assert old_ev.priority == new_ev.priority, f"Priority mismatch: {old_ev.priority} != {new_ev.priority}"

                    # Optional attributes depending on the event type
                    assert getattr(old_ev, 'is_aoe_targeting', False) == getattr(new_ev, 'is_aoe_targeting', False), "is_aoe_targeting mismatch"
                    assert getattr(old_ev, 'is_spell_sequence', False) == getattr(new_ev, 'is_spell_sequence', False), "is_spell_sequence mismatch"
                    assert getattr(old_ev, 'aura_id', None) == getattr(new_ev, 'aura_id', None), "aura_id mismatch"
                    assert getattr(old_ev, 'aura_origin_spell_id', None) == getattr(new_ev, 'aura_origin_spell_id', None), "aura_origin_spell_id mismatch"

                # 3. Only insert old events into the heap so game logic does not double-fire
                for cascading_event in old_events:
            ...
            self._targeting_system.apply_targeting_event(timestamp, source_id, spell.spell_id, target_id)
    ...
    def _fetch_cascading_events(self, u_event: UpcomingEvent, new_obj: Optional[GameObj], source: GameObj, spell: Spell, target: GameObj, new_aura_id: int) -> Iterable[UpcomingEvent]:
        ...
        if spell.is_area_of_effect and not u_event.is_aoe_targeting:
            ...
            #DEBUGGING
            lst_old_target_ids =list(self._select_targets_for_aoe(source, target, self.view_game_objs))
            lst_new_target_ids =list(self._targeting_system.select_targets_for_aoe(source.obj_id, target.obj_id))
            assert lst_old_target_ids == lst_new_target_ids, f"aoe_targeting mismatch between {lst_old_target_ids} and {lst_new_target_ids}"
    ...
    def _new_decide_outcome(self, timestamp: int, source_id: int, spell_id: int, target_id: int, expired_aura: bool, is_aoe_targeting: bool) -> Outcome:
    ...
    def _decide_outcome(timestamp: int, source_obj: GameObj, spell: Spell, target_obj: GameObj, expired_aura: bool, is_aoe_targeting: bool) -> Outcome:
        ...
        if not is_aoe_targeting:  # This was previously validated if AoE
            if not source_obj.is_valid_source:
                return Outcome.SOURCE_IS_DISABLED
            if not OldWorldState._gcd_is_available(timestamp, source_obj, spell):
                return Outcome.GCD_NOT_READY
        # Validate target
        if not target_obj.is_valid_target and not source_obj.obj_id == target_obj.obj_id:
            return Outcome.TARGET_IS_INVALID
        # Validate source relative to target
        if not OldWorldState._is_within_range(source_obj, spell, target_obj):
            return Outcome.OUT_OF_RANGE
        # More outcome conditions to be added here.
        return Outcome.SUCCESS
    ...
    def handle_spawn(self, timestamp: int, source: GameObj, spell: Spell, target_id: int) -> Optional[GameObj]:
        ...
        self._targeting_system.spawn_game_obj(timestamp, parent_id, child.obj_id, spell.spell_id, target_id)
        self._vfx_and_sfx_system.spawn_game_obj(child.obj_id, spell.spell_id)
        self._update_default_ids(child, spell)
        return child

    @staticmethod
    def modify_game_obj(timestamp: int, source: GameObj, spell: Spell, target: GameObj) -> None:
        OldWorldState._apply_source_effects(spell, timestamp, source, target)
        OldWorldState._apply_target_effects(spell, source, target)
    ...
    def _new_fetch_cascading_events(
            self,
        ...
        if new_obj_id is not None:
            # We can retrieve the newly spawned object's current target directly from the targeting system's ECS data.
            # Its spawn timestamp is simply the timestamp of the event that created it.
            ...
            for s_id, trigger_timestamp, priority in scripted_spells:
                yield UpcomingEvent(
                    event_id=self._new_event_id_gen.generate_new_id(),
        ...
        if self._targeting_system.is_area_of_effect(spell_id) and not u_event.is_aoe_targeting:
            ...
            for aoe_target_id in target_ids:
                priority += 1
                aoe_copy = u_event.create_copy()
                aoe_copy.event_id = self._new_event_id_gen.generate_new_id()
                aoe_copy.priority = priority
        ...
        if sequenced_spells is not None:
            ...
            for next_spell_id in sequenced_spells:
                priority += 1
                seq_copy = u_event.create_copy()
                seq_copy.event_id = self._new_event_id_gen.generate_new_id()
                seq_copy.priority = priority
    ...
    def debug_compare_ecs_to_gameobj(
        ...
        targeting_data: 'ObjTargetingData',
        controls_data: 'ObjControlsData',
        ...
        reconstructed._loadout = controls_data.loadout
        ...
        # Combat data
        reconstructed.parent_id = targeting_data.parent_id
        reconstructed._state = targeting_data.status
        reconstructed.gcd_mod = combat_data.gcd_mod
        reconstructed.current_target = targeting_data.current_target_id
